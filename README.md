...
- JWT 是一种 *紧凑* 且 *自包含* 的，用于在多方传递 JSON 对象的技术
    - 紧凑：数据小，可以通过 URL、POST 参数、请求头发送，而且因为数据较小所以传输速度快
    - 自包含：使用 payload 数据块记录用户必要且不隐私的数据，可有效减少数据库访问次数，提高代码性能
- 传递的数据可以使用数字签名来增加其安全性，可以使用 HMAC 加密算法或者 RSA 公钥/私钥 加密方式
- JWT 一般用于处理用户身份认证或者数据信息交换
    - 用户信息认证：一旦用户登录后，每个后续的请求都必须包含 JWT，允许用户访问该令牌允许的路由、服务和资源。单点登录是当今广泛使用 JWT 的一项功能，因为 JWT 的体积很小，而且可以轻松的跨不同的域使用
